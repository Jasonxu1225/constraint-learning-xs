...
class RolloutBufferWithCost(BaseBuffer):
    def __init__(
        ...
        self.observations, self.orig_observations, self.actions, self.new_actions, self.rewards, self.advantages = None, None, None, None, None, None
    ...
    def reset(self) -> None:
        ...
        self.new_orig_observations = np.zeros((self.buffer_size, self.n_envs) + self.obs_shape, dtype=np.float32)
        self.actions = np.zeros((self.buffer_size, self.n_envs, self.action_dim), dtype=np.float32)
        self.new_actions = np.zeros((self.buffer_size, self.n_envs, self.action_dim), dtype=np.float32)
        self.dones = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
        self.log_probs = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
    ...
    def add(self, obs: np.ndarray, orig_obs: np.ndarray, new_obs: np.ndarray, new_orig_obs: np.ndarray,
            action: np.ndarray, new_action: np.ndarray, reward: np.ndarray, cost: np.ndarray, orig_cost: np.ndarray,
        ...
        :param new_obs: Next observation (to which agent transitions)
        :param action: Action
        :param new_action: newt predicted Action
        :param reward:
        :param cost:
        ...
        self.new_orig_observations[self.pos] = np.array(new_orig_obs).copy()
        self.actions[self.pos] = np.array(action).copy()
        self.new_actions[self.pos] = np.array(new_action).copy()
        self.dones[self.pos] = np.array(done).copy()
        self.log_probs[self.pos] = log_prob.clone().cpu().numpy()
        self.rewards[self.pos] = np.array(reward).copy()
    ...
    def get(self, batch_size: Optional[int] = None) -> Generator[RolloutBufferSamples, None, None]:
        ...
        # Prepare the data
        if not self.generator_ready:
            for tensor in ["orig_observations", "observations", "actions", "new_actions", "log_probs", "reward_values",
