...
class DistributionalActorTwoCriticsPolicy(ActorCriticPolicy):
    ...
    def dis_build_QRDQN(self, lr_schedule: Callable[[float], float], N, tau_update, LR_QN, qnet_layers,
        ...
        self.cost_value_net_local = QRDQN(self.qnet_layers, self.features_dim, action_dim, self.N)
        self.cost_value_net_target = QRDQN(self.qnet_layers, self.features_dim, action_dim, self.N)
    ...
    def dis_build_IQN(self, device, lr_schedule: Callable[[float], float], N, tau_update, LR_QN,
        ...
        self.cost_value_net_local = IQN(self.qnet_layers, self.features_dim, action_dim, self.N, device)
        self.cost_value_net_target = IQN(self.qnet_layers, self.features_dim, action_dim, self.N, device)
    ...
    def dis_build_SplineDQN(self, device, lr_schedule: Callable[[float], float], N, tau_update, LR_QN,
        ...
        #     self.cost_value_net_local = IQN(self.features_dim, self.qnet_layers, self.n_cos, self.N, device)
        #     self.cost_value_net_target = IQN(self.features_dim, self.qnet_layers, self.n_cos, self.N, device)
        self.cost_value_net_local = SplineDQN(self.qnet_layers, self.features_dim, action_dim, self.N, device)
        self.cost_value_net_target = SplineDQN(self.qnet_layers, self.features_dim, action_dim, self.N, device)
    ...
    def dis_build_NCQR(self, device, lr_schedule: Callable[[float], float], N, tau_update, LR_QN,
        ...
        #     self.cost_value_net_local = IQN(self.features_dim, self.qnet_layers, self.n_cos, self.N, device)
        #     self.cost_value_net_target = IQN(self.features_dim, self.qnet_layers, self.n_cos, self.N, device)
        self.cost_value_net_local = NCQR(self.qnet_layers, self.features_dim, action_dim, self.N, device)
        self.cost_value_net_target = NCQR(self.qnet_layers, self.features_dim, action_dim, self.N, device)
    ...
    def forward(self, obs: th.Tensor, deterministic: bool = False) -> Tuple[th.Tensor, th.Tensor, th.Tensor]: #important-----------------
        ...
        if self.method == 'QRDQN' or self.method=='SplineDQN' or self.method=='NCQR':
            distributional_cost_values = self.cost_value_net_local(features, actions)
            # if self.input_action:
            ...
            distributional_cost_values, _ = self.cost_value_net_local(features, actions)
