...
from stable_baselines3.common.policies import QRDQN
...
class DistributionalPolicyIterationLagrange(ABC):
    ...
    def __init__(self,
        ...
        # Z(s, a)
        if self.recon_obs:
            self.qnetwork_local = QRDQN(self.height * self.width + self.n_actions, self.qnet_layers, self.N)
            self.qnetwork_target = QRDQN(self.height * self.width + self.n_actions, self.qnet_layers, self.N)
        else:
            self.qnetwork_local = QRDQN(self.env.observation_space.shape +
                                        get_action_dim(self.env.action_space), self.qnet_layers, self.N)
            self.qnetwork_target = QRDQN(self.env.observation_space.shape +
                                         get_action_dim(self.env.action_space), self.qnet_layers, self.N)
                ...
                for action in range(self.n_actions):
                    ...
                    if self.recon_obs:
                        recon_states = torch.tensor(self.idx2vector(states, height=self.height, width=self.width)).to(
                            torch.float32)
                        recon_actions = torch.nn.functional.one_hot(torch.tensor(action), num_classes=self.n_actions)
                    else:
                    ...
                    with torch.no_grad():
                        dis_costs = torch.mean(torch.cat([recon_states, recon_actions], dim=1), dim=1)

                    current_penalty = self.dual.nu().item()
                    # lag_costs = self.apply_lag * current_penalty * orig_costs[0]
                    lag_costs = self.apply_lag * current_penalty * dis_costs[0]
                    # Get value
    ...
    def bellman_update(self, old_v, x, y, cost_function):
        ...
        for action in range(self.n_actions):
            ...
            if self.recon_obs:
                recon_states = torch.tensor(self.idx2vector(states, height=self.height, width=self.width)).to(torch.float32)
                recon_s_primes = torch.tensor(self.idx2vector(s_primes, height=self.height, width=self.width)).to(torch.float32)
                recon_actions = torch.nn.functional.one_hot(torch.tensor(action), num_classes=self.n_actions)
            else:
                recon_states = torch.tensor(states).to(torch.float32)
            ...
            if len(recon_actions.shape) != len(recon_states.shape):
                recon_actions = recon_actions.view(recon_states.shape[0], -1)
            with torch.no_grad():
                Q_targets_next = self.qnetwork_target(torch.cat([recon_s_primes, recon_actions], dim=1))
                Q_targets_next = Q_targets_next.unsqueeze(-1).transpose(1,2)

            q_costs = torch.tensor(orig_costs).view(-1, 1)
            q_dones = torch.tensor(int(dones)).view(-1, 1)

            Q_targets = q_costs.unsqueeze(-1) + (self.gamma * Q_targets_next * (1 - q_dones.unsqueeze(-1)))
            Q_expected = self.qnetwork_local(torch.cat([recon_states, recon_actions], dim=1)).unsqueeze(-1)
